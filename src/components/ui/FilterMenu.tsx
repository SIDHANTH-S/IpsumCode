...
function useSmartSticky(triggerRef: React.RefObject<HTMLDivElement | null>, isOpen: boolean) {
  ...
  useEffect(() => {
    ...
    baseTop = Math.max(66, Math.min(baseTop, window.innerHeight - popupHeight - 16))
...
function CustomSelect({
  ...
  const getDisplayValue = () => {
    if (multiple) {
      const arr = Array.isArray(value) ? value : []
      if (arr.length === 0) return <span className="opacity-40">{placeholder}</span>
      if (arr.length === 1) return options.find((o) => o.value === arr[0])?.label || arr[0]
      return `${arr.length} selected`
    }
    const selected = options.find((o) => o.value === value)
    return selected ? selected.label : <span className="opacity-40">{placeholder}</span>
  }
...
export function FilterMenu({
  ...
  return (
    <>
      <div className="relative inline-block cursor-pointer" ref={triggerRef} onClick={() => setIsOpen(!isOpen)}>
        {trigger}
      </div>
      ...
      {isOpen && createPortal(
        ...
        >
          ...
          <div className="flex shrink-0 items-center gap-2 border-b border-neutral-700/50 p-4 pb-3">
            <span className="text-sm font-medium text-neutral-200">Match</span>
            <CustomSelect
              className="w-[72px]"
              value={matchType}
          ...
          <div className="flex-1 overflow-y-auto p-4 [&::-webkit-scrollbar]:hidden">
            <div className="space-y-3">
              {filters.map((f) => {
                const def = availableFilters.find((d) => d.id === f.definitionId)
                if (!def) return null
                const Icon = def.icon
                ...
                return (
                  <div key={f.id} className="flex items-center gap-2 pl-1">
                    ...
                    <CustomSelect
                      ...
                      value={f.operator}
                      onChange={(v) => handleUpdateFilter(f.id, { operator: v })}
                      options={[
                        { value: "is", label: "is" },
